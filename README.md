...
- 按当前 Adapter 声明的设备形态发布电量：左右耳与充电盒、头戴式整机，或 Android 系统整机。
- 仅在私有协议确认成功后发布降噪、关闭、通透和型号专属模式；未确认时不发布对应的私有控制。
- 耳机卡片的“更多设置”优先打开 Adapter 声明且可启动的厂商控制 App；该选项关闭或 App 不可用时，
  打开真实蓝牙设备详情。

### 会话与控制权

- 每台已连接耳机独立维护识别、传输、协议和 MiLink 发布状态，应用按设备会话展示这些状态。
...
- 在设置页配置厂商设置跳转、运行时退避和模块暂停。
- 提供日志导出，以及需要 Root 的重启 MiLink、重启蓝牙和停止厂商 App 操作。
