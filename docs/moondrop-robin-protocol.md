...
`PUBLISH_AFTER_WRITE_THEN_REFRESH`：控制帧写入成功后先发布用户选择的目标模式，等待
`600 ms` 执行第一次只读模式查询。若回报尚未达到目标，Robin Adapter 暂缓该观测进入公开
状态，并按 `500/700/900/1200 ms` 的有界序列继续查询；回报目标模式后立即结束确认，达到
上限仍不一致则接受最后真实回报，由统一状态链自然矫正卡片。后续确认只发送查询帧，不重复
发送设置帧，也不创建常驻轮询。
...
- `MoondropRobinAdapter`：名称判型、传输候选、能力确认、电量启动确认、模式写后确认和恢复决策；
