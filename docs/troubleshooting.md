...
如果“识别”未完成，通常是设备未被保守判定为耳机或型号名称未命中；如果“发布”未
完成，则重点检查 MiLink 进程是否加载了模块。
...
- 标准蓝牙耳机本来就只发布系统电量和音量；
- 需要私有 GATT/RFCOMM 的型号必须等“通道”和“协议”完成；
...
先区分耳机协议通道与 MiLink 设备共享通道：HyperEars 的 GATT/RFCOMM 通道只负责耳机
电量/模式，不承载跨设备音频流转。若两个方向表现不一致：
