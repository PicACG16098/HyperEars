...
- **打开厂商设置**：有已安装且可启动的匹配控制 App 时优先打开它；关闭后
  始终进入真实蓝牙设备详情。
...
设置页还提供四个需要 Root 的操作：导出日志、重启 MiLink、重启蓝牙、停止已支持的厂商
控制 App。没有 Root 时按钮显示为禁用状态；快捷控制结果不常驻显示在设置页，只在“详细
日志”开启时写入应用日志。重启蓝牙会断开当前蓝牙设备，停止控制 App 会触发其 Binder
death，从而释放外部控制权。
