...
页序，点击底部项目或左右滑动都会切换页面并同步选中状态；关于页不订阅蓝牙运行状态，手动
更新检查也只访问 GitHub Releases。外层只负责底部导航和分页；每个页面独立拥有自己的 `Scaffold`、
标题栏、列表状态和折叠滚动行为，页面之间不会共享标题栏展开偏移。设置页写入策略后立即
由 libxposed `RemotePreferences` 通知已注入进程，不依赖页面切换、广播或轮询。
“调试”是设置页的二级页面，“适配器”是调试页的下一级页面；两者各自拥有独立标题栏与
滚动状态。进入这些页面时隐藏底部导航，系统返回键与标题栏返回键按页面层级逐级返回。
...
主页模式控件只提交一次类型化控制请求，后续显示由会话状态快照更新。Bluetooth 进程仅在状态实际变化
