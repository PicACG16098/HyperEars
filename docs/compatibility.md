...
| 设备范围 | 层级 | 证据 | 识别与确认 | 电量 | 噪声模式 |
|---|---|---|---|---|---|
| 荣耀亲选耳机 X5s Pro（BTV-ME10） | 具体型号 | 实机验证 | 规范化型号精确匹配 | 组件（SPP 协议确认后） | 降噪、关闭、通透 + 四档深度（类型化扩展） |
| 其他荣耀系 TWS | 家族候选（待引入） | 未验证 | 名称含 honor/荣耀 | 系统整机回退 | 无（待多型号协议证据） |

私有能力按"标准能力起步、有效协议响应确认"开放：组件电量在收到合法电量帧后开放，
三态模式在收到合法状态帧后开放；降噪深度以 `honor.x5spro` 类型化状态与控制接入
v2.0.0 传输架构。未获有效响应时保持系统标准回退。详见
