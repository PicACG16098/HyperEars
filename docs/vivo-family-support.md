...
1. **家族通用能力**：名称命中 vivo/iQOO TWS 家族后，建立 vivo GAIA
   RFCOMM 通道，读取 `0x0207/0x8207` 左右耳/盒电量，并开放
   `0x0130/0x0230` 降噪、关闭、通透三态。
2. **具体型号差异**：具体 Adapter 只覆盖已有证据的 GAIA 版本、查询载荷、
   设置后缀或端点差异，不复制通用协议状态机。

家族默认画像来自公开手工逆向，并由 Air3 Pro 实测和 TWS 3e 项目交叉确认
公共 vendor、命令号及三态编号。型号名称只负责 Adapter 选择，Profile 只负责
...
| 其他 vivo TWS | `VivoEarbudAdapter` | 家族默认 v4，`mode 03 01` | 私有电量、三态降噪、流转、音量 |
| iQOO TWS 家族 | `VivoEarbudAdapter` | 家族默认 v4，`mode 03 01` | 私有电量、三态降噪、流转、音量 |
...
`vivo TWS Air2`。目录用于诊断与后续 Profile 覆盖；未来未登记但保持
`vivo TWS` 或 `iQOO TWS` 命名格式的型号也会选择家族默认画像。

## 协议画像

| Profile | GAIA | 降噪查询载荷 | 降噪设置载荷 | 绑定状态 |
|---|---:|---|---|---|
...
Profile 是不可变字节配置，不包含零售名称、MiLink 能力或蓝牙连接代码。
家族 Adapter 选择默认 Profile；增加可参数化的新协议差异只需登记 Profile，
并由具体型号 Adapter 覆盖选择，无需复制一套 `EarbudProtocol`。

## Air2 限制

公开 BlueZ 记录确认 Air2 暴露 `0837/0838` 自定义 GATT 服务，但这不能证明
它一定同时暴露当前 Classic RFCOMM 入口。当前按 vivo 家族默认画像尝试
RFCOMM；若真机报告确认 Air2 仅支持 GATT，则为 Air2 增加独立传输 Profile，
