...
相对 `1.2.0`，本版主要完善模块运行时、控制权协调、设置与诊断界面；没有新增具体耳机协议型号。

### Added

- 为 Adapter 增加厂商控制 App 声明和优先级目录，收录 11 个已确认的 Android 包名；控制 App
  仅作为控制权边界，不参与耳机型号判定。
- 增加控制 App 进程 Hook：通过进程 Binder 存活登记感知厂商 App 是否运行，并支持多个候选
  App 按 Adapter 声明顺序选择当前所有者。
- 增加设置页和 Material 3 页面框架，提供模块暂停、厂商设置跳转、运行时退避和详细日志开关。
- 增加应用内日志导出：通过系统文件选择器合并经过模块标识过滤的 LSPosed 守护进程日志与
  HyperEars 应用滚动日志。
- 增加 Root 快捷操作：重启 MiLink、重启蓝牙、停止已登记的厂商控制 App；执行结果通过诊断
  日志记录，不常驻设置页。

### Changed

- 将设备会话生命周期扩展为系统连接、私有传输、协议确认和控制权四个维度；每个物理设备
  继续使用独立的 Adapter 与 `ProtocolSession`。
- 当已启用退避且对应厂商 App 被 Hook 时，模块让出私有 GATT、RFCOMM 或 L2CAP 控制通道，
  只保留 MiLink 流转、系统音量、标准蓝牙和系统整机电量；控制权通过 Binder death 恢复，
  不依赖厂商 App 是否自行建立蓝牙连接。
- “更多设置”改为优先启动当前 Adapter 声明且可用的厂商控制 App；未启用该选项或 App 不可用
  时，回退到真实蓝牙设备详情页。
- 增加模块运行时闸门和 RemotePreferences 同步；暂停模块会清理 HyperEars 的发布状态，
  不关闭 Android 原生蓝牙或音频路由。
- 详细日志关闭时不产生模块诊断记录；开启后，注入进程日志写入 LSPosed 守护进程，设置变更
  和快捷操作结果写入应用本地有界滚动日志，导出时合并为一个文件。
- 重构主页、设置和关于页面为独立的 Material 3 页面；每页独立维护标题栏和滚动状态，主页按
  设备会话展示识别、传输、协议、能力和 MiLink 发布状态。仅设置开关提供轻触反馈，底部导航
  和普通按钮不增加额外触感。
- 移除正式模块和协议测试工具未使用的 Compose tooling 依赖，并从正式 APK 排除协程
  `DebugProbesKt.bin` 调试资源；加入 baseline profile。

### Documentation

- 新增厂商控制 App 与 LSPosed 作用域目录，统一记录显示名、包名、Adapter 声明顺序、页面
  跳转条件、运行时退避条件和退出恢复语义。
- 统一中英文 README、兼容性矩阵、安装、排障、架构、隐私、贡献和第三方声明，明确控制 App
  不参与判型、私有能力确认条件和日志隐私边界。
- 修正设置名称、生命周期、模式能力、标准回退和控制权描述，补充模块暂停与 Root 操作说明。
- 新增 Markdown 结构、站内链接、标题锚点、YAML、控制 App 目录与 `scope.list` 一致性校验，
  并接入 CI。

### Verification

- 新增控制权仲裁、作用域、设置同步和诊断导出单元测试。
- 本地 `testDebugUnitTest`、`:system-module:lintRelease` 和 `:system-module:assembleRelease` 通过。
- Markdown、链接锚点、YAML、控制 App 目录与 `scope.list` 校验通过；目录包含 11 个控制 App。
- GitHub Actions 完成正式签名 APK 构建、签名校验、SHA-256 生成和 Release 发布。
