...
- 增加 MOONDROP Robin / 水月雨知更鸟的公开协议适配。通过精确蓝牙名称或品牌与型号组合
  选择候选，严格握手后开放左右耳电量以及降噪、关闭、通透三态控制。
- **荣耀亲选耳机 X5s Pro（BTV-ME10）**：新增已实机验证的具体型号适配。通过私有 RFCOMM SPP
  `5A 00` 帧协议确认后，发布左右耳与充电盒组件电量（含充电标志）以及降噪、关闭、通透三态；
  降噪深度（智能/轻度/中度/深度）以类型化扩展状态与控制接入 v2.0.0 传输架构。协议资料见
  `docs/honor-x5s-protocol.md`。

### 兼容性与可靠性

- 标准 SPP UUID 仅作为 Robin 的 RFCOMM 连接端点，不参与品牌或型号判定，避免具有同一
  公共服务的无关耳机被误接管。
- Robin 模式设置后主动查询设备状态，并只发布设备回读结果。精确型号握手失败时保留原
  Adapter，沿用有界退避、休眠与生命周期唤醒机制。
